...
const CARD_BG = "#F4E7E1";   // beige
const PAGE_BG = "#FAF7F3";   // overall page
...
function UserDashboard() {
  ...
  return (
    <div className="w-screen min-h-screen flex flex-col gap-5 items-center" style={{ backgroundColor: PAGE_BG }}>
      <Nav />

      {/* 🔎 Search Results Section */}
      {searchItems && searchItems.length > 0 && (
        <div style={{ backgroundColor: CARD_BG }} className="w-full max-w-6xl flex flex-col gap-5 items-start p-5 shadow-md rounded-2xl mt-4">
        ...
        <div className="relative w-full">
          {showCateLeft && (
            <button
              ...
              className="absolute left-0 top-1/2 -translate-y-1/2 text-white p-2 rounded-full shadow-lg hover:opacity-90 z-10"
            >
              <FaChevronLeft />
            </button>
          )}
          <div
            ref={cateScrollRef}
            className="w-full flex overflow-x-auto gap-4 pb-2 scrollbar-thin scrollbar-thumb-[#347928] scrollbar-track-transparent scroll-smooth"
          >
          ...
          {showCateRight && (
            <button
              ...
              style={{ backgroundColor: PRIMARY }}
              className="absolute right-0 top-1/2 -translate-y-1/2 text-white p-2 rounded-full shadow-lg hover:opacity-90 z-10"
            >
              <FaChevronRight />
            </button>
          )}
